...
export function ConfirmStep({ state, onBack }: ConfirmStepProps) {
  ...
  if (submitted) {
    ...
    const whatsappMsg = `You've received a payment via Bridgelet! Claim it here: ${claimUrl}`;
    ...
    return (
      ...
      >
        ...
        <div className="mt-4 pt-4 border-t border-green-200/60">
          <p className="text-sm font-medium text-green-800 mb-2">Or share the link directly:</p>
          <a
            href={whatsappLink}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 rounded-md bg-[#25D366] px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-[#20bd5a] focus:ring-2 focus:ring-[#25D366] focus:ring-offset-2"
          >
